...
##            del layer_metadata
            ...
            layer_metadata = md.Metadata(fc)
            print("\t\t {0}".format(layer_metadata.title))
            export_19139_path = workspacepath+"/Export Metadata/"+fc+".xml"
            layer_metadata.exportMetadata(export_19139_path,"ISO19139","EXACT_COPY")

            if not layer_metadata.isReadOnly:
                # Import standard-format metadata content to the target item
                layer_metadata.importMetadata(export_19139_path, 'DEFAULT')
                layer_metadata.save()
            ...
            del layer_metadata
            if os.path.isfile(export_19139_path) or os.path.islink(export_19139_path):
                os.unlink(export_19139_path)
