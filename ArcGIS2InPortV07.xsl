...
<xsl:stylesheet version="1.0" exclude-result-prefixes="esri res t msxsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.esri.com/xslt/translator" xmlns:esri="http://www.esri.com/metadata/" xmlns:res="http://www.esri.com/metadata/res/" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template match="/">
		...
		<xsl:variable name="defaultEffectiveDate" select="'[defaultEffectiveDate]'"/>		
		<!-- these variables only need to be filled in if you don't have the various contact roles defined in your metadata -->
		<xsl:variable name="defaultPointOfContactEmail" select="'[defaultPointOfContactEmail]'" />
		<xsl:variable name="defaultDataStewardEmail" select="'[defaultDataStewardEmail]'" />
		<xsl:variable name="defaultMetadataContactEmail" select="'[defaultMetadataContactEmail]'" />
		<xsl:variable name="defaultOrganizationName" select="'[defaultOrganizationName]'" />
		...
		<inport-metadata version="1.6">
			...
			<extents>
				...
				<extent>
				    ...
				    <description><xsl:value-of select="/metadata/dataIdInfo/dataExt/exDesc"/></description>
					<time-frames>
			...
			<!-- added/modified TJH -->
				<xsl:for-each select="metadata/distInfo/distTranOps/onLineSrc">			
					<distribution>
						...
						<file-type><xsl:value-of select="/metadata/distInfo/distFormat/formatName"/></file-type>
